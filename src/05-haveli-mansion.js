...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    const validRole = ["malik", "naukar", "mehman"];
  ...
  removeResident(name, passcode) {
    if(this.passcode === passcode){
      if(this.residents.some((r) => r.name === name)){
        ...
        this.residents.push(this.residents.filter((r) => r.name !== name));
        return { success: true, message: `${resident.name} ko haveli se nikal diya!` }
      }else{
