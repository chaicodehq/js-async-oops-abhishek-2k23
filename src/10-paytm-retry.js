...
export async function retryPayment(paymentFn, maxRetries, delayMs) {
  if (maxRetries < 0) {
    throw Error("Invalid maxRetries")
  }
  if (delayMs <= 0) {
    throw Error("Invalid delaysMs")
  }
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      if (attempt >= maxRetries) {
        throw Error("Network error during transaction")
      }
  ...
  try {
    ...
    try {
      ...
      throw new PaymentError(
        `Primary: ${primaryError.message}, Fallback: ${e.message}`,
        "BOTH_FAILED",
        0,
      )
    }
