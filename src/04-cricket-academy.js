...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    const totalAvg = this.innings.reduce((t, it) => it.strikeRate + t, 0);

    let avgSR = ((totalAvg / this.innings.length));

    return avgSR;
  }

  getProfile() {
    let totalRuns = this.innings.reduce((totalRun, inn) => inn.runs + totalRun, 0);
...
export class Bowler extends Player {
  ...
  getEconomy() {
    const total = this.spells.reduce((t, spell) => {
      t.runsConceded += spell.runsConceded;
      t.overs += spell.overs;
      return t;
    }, {runsConceded: 0, overs: 0})

    const avgEconomy = ((total.runsConceded/ total.overs));

    return avgEconomy;
  }

  getProfile() {
    const totalWickets = this.spells.reduce((tw, spell) => spell.wickets + tw, 0);
...
export class AllRounder extends Player {
  ...
  getEconomy() {
    const total = this.spells.reduce((t, spell) => t + spell.economy, 0)

    const avgEconomy = ((total/this.spells.length) ) || 0;
  ...
  getProfile() {
    ...
    return {
      ...super.getProfile(), battingStyle: this.battingStyle,  bowlingStyle: this.bowlingStyle, role: "allrounder", totalRuns, totalWickets, inningsPlayed: this.innings.length, spellsBowled: this.spells.length
    }
  }
}
